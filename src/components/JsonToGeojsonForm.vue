  ...
  <form>
    ...
    <div class="form-row align-items-center" v-if="showWgs84Alert">
      <div class="col">
        <div class="alert alert-warning" role="alert">
          It looks like your FeatureSet has features with geometries with
          spatial references not in Latitude/Longitude (WGS84). In general,
          GeoJSON only works with WGS84, which is why the map probably looks
          wrong, so you probably want to change your input data to have that
          spatial reference. Try setting the "Output Sptaial Reference" to 4326.
        </div>
      </div>
    </div>
    ...
    <div class="form-group">
      <textarea
        rows="10"
        class="form-control"
        v-show="showResultArea"
        v-model="resultJsonString"
      ></textarea>
    </div>

    <div class="form-row" v-show="showResultArea">
      <div class="col d-flex align-items-center justify-content-between">
        <label class="m-0" v-show="showResultArea"
          >Pretty Print <input type="checkbox" v-model="prettyPrint"
        /></label>
        <button
          type="button"
          class="btn btn-primary"
      ...
      if (inputJson) {
        try {
          ...
          if (
            ...
            Object.prototype.hasOwnProperty.call(
              inputJson.spatialReference,
              "wkid"
            ) &&
            inputJson.spatialReference.wkid !== 4326
          ) {
